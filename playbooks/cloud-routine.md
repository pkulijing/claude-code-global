...
`skills/routine-docs/SKILL.md` 的「无人值守分岔契约」表是这条的现成范例：

| 分岔                 | 有人在环时   | 无人值守时                                             |
| -------------------- | ------------ | ------------------------------------------------------ |
| 发现该走 `/start`    | 反问用户     | 跳过该条、记入 PR 的跳过清单                           |
| review 满 2 轮未收敛 | 停下交回用户 | 停止修复、未收敛项写进 PR 描述（PR 就是那个人工闸口）  |
| review 降级          | 告知用户     | 照常继续，但在 PR 描述如实标注「未经独立 review 把关」 |
| lint 失败            | 停下问用户   | 放弃该条、`git restore`、记入跳过清单                  |
| push / 开 PR 失败    | 问用户       | 放弃该批，不重试（下次运行会重新捡起）                 |
