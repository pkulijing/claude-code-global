...
| `/commit`              | 分析当前变更，自动生成中文 semantic commit message 并提交，末尾按执行 Agent 附加 Co-authored-by（CC → `Claude` / Codex → `OpenAI Codex`）；**提交前自动内嵌 `/review-loop`**（默认 CC `/code-review`、复杂 diff 升级 codex，迭代到「运行验证通过 + 无高置信 correctness 问题」才落 commit）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `/review-loop`         | 提交前的自动 review 迭代环。**收敛靠「运行验证 + 高置信过滤」，非 reviewer 挑不出为止**——治两个实战病根（挑刺变慢、把基础功能审废）。**分层 reviewer**：默认走快而低噪的 CC `/code-review`（自带 verification 过滤误报），仅并发/多线程/跨进程重试/状态机/难复现/跨 3+ 模块 diff 才升级引 codex 独立 review（`--codex`/`--cc` 手动覆盖；升级档「审的模型 ≠ 写 diff 的模型」才算独立，CC 写→codex 审）。**三要素并闸收敛**：(A) 运行验证（受影响测试全绿 + happy-path 主流程跑通，编排器无测先补；排在 reviewer 意见之前，堵「基础功能审废无人知」）+ (B) 无高置信 correctness finding（对齐官方 ≥80 阈值，只认 file:line 证据+真会触发，pre-existing/pedantic/推测 corner case 不阻断）+ (C) 已定前提未被重复质疑。修复走 TDD 正序（先写会红的复现测试再改实现），每 3 轮一个人工闸口。降级：codex 不可用回退 CC `/code-review`、再不行降级本会话自审并标注；配置/指令文件不跳过，仅纯用户文档/注释才跳过。由 `/commit` 提交前自动调用，也可手动跑 |
...
- 工作流：`/backlog` 起新想法 → `/start <issue#>` 开新轮 → 执行中 Agent 自主 `/commit` 收口（每次 commit 前自动经 `/review-loop`：默认 CC `/code-review`、复杂 diff 升级 codex，迭代到「运行验证通过 + 无高置信 correctness 问题」）→ `/finish` 收尾时 PR/commit 写 `Closes #N` 自动关 issue（GitHub / GitLab 均原生支持）
