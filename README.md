...
| `/review-loop`         | 提交前的自动 review 迭代环：**审的模型 ≠ 写的模型才算独立**——CC 写代码时由 codex（`/codex:review`）独立 review 整个工作树，Codex 写代码时 codex 非独立、降级本会话自审并标注。发现正确性问题就修 → 复审 → 迭代至 clean（收敛看「是否真会出错」而非 P 级数字，自动修复最多 3 轮）。codex 不可用同样降级自审；配置 / 指令文件不跳过，仅纯用户文档 / 注释才跳过。由 `/commit` 提交前自动调用，也可手动跑                                                    |
