...
| **lark-cli 文档创作规则** | 指针到 [`rules/lark.md`](rules/lark.md)：lark-cli 创作飞书云文档默认加署名行（`⚡ Crafted with lark-cli · <YYYY-MM-DD>`）+ docx 实操技巧（署名落位 / 媒体置顶 / 内容文件相对路径）+ 授权与 scope 管理（禁 `--domain` 打包 / 预检凑最小集 / 审批中 scope 先拆 / 换机重认证 / 无人值守走 bot） |
