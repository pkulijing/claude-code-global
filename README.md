...
**两条 routine 的撞车防线**：`/routine-docs` 每天跑、也写 `playbooks/*.md`，PR 可能在人手上挂好几天——**光靠 cron 时间错开不够**。故 `/routine-slim` 每次运行强制列出所有 open PR、把它们碰过的文件整体排除；**列不出 open PR 就中止本次运行**。
