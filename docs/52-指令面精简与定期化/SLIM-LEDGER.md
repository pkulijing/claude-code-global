...
## 阶段 5 · `/routine-slim`（+7,030 新增，非精简）

本阶段是**新增**而非精简，无删减可记账。它把本轮实操中真正可机械化的判据固化下来：

- 阈值闸用的就是本轮阶段 0 写的 `scripts/context_budget.py delta --threshold 15`；
- 三板斧的两张清单（允许删除 5 条 / 禁止删除 4 条）**逐字来自本账本顶部**，是本轮真刀真枪用过一遍的判据，不是纸上推演；
- 「分层优先于删除」「安全内容规则留原地、推导才移走」两条来自阶段 4 的实操；
- PR 描述强制三列表格 + **保留未动清单**（反向证据，证明禁止删除清单被真的执行过），来自本账本的结构。

`CLAUDE.md` 同步从 4,593 → 5,230（**净增**）：安全边界段由一条 routine 扩为两条，并写明二者输入不同故规则不同；`scripts/` 条目补上 `context_budget.py`。这是有意的增加 —— 安全边界的说明属禁止删除类。
