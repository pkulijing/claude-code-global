...
4. **提交前 review loop**（防止 bug 随 commit 沉淀，尤其并发 / 复杂逻辑）：调 `/review-loop` 对当前工作树跑自动 review 迭代，**迭代到 clean 才继续往下**。细节以 `/review-loop` 为单一真源，简言之：**委派一个子 agent 跑 CC 自带 `/code-review`**——默认 `sonnet × medium`，diff 命中并发 / 多线程 / 跨进程重试 / 状态机 / 难复现 / 跨 3+ 模块编排等复杂特征才升 `opus × high`（永远显式传档位、永远委派、不用其他组合）。发现**高置信 correctness 问题**（附 file:line 证据、真会触发）自动修复→**跑测试 + happy-path 验证**→复审，迭代到「运行验证通过 + 无高置信 correctness 问题」才 clean（pre-existing / pedantic / 推测式 corner case 不阻断）。琐碎改动自动跳过（配置 / 指令文件不跳）；自动修复每满 2 轮强制停下交回用户（人工闸口，授权后再来至多 2 轮）。降级链：委派失败→主会话直跑同档位 `/code-review`；连它都不可用才停下告知用户降级本会话自审。`/review-loop` 自动修复不停下逐条等用户确认，并把迭代留痕到 `docs/<N>-*/REVIEW.md`（如有）。放在 lint 之前，是因为 review-loop 会自动改代码——须让下一步 lint 覆盖到这些修复。
