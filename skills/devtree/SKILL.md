...
| 3    | **Epic 结构** | 作者主导，AI 协助 | Markdown 树形列表，定义目标层次和每个叶 Epic 包含的开发项编号 |

另外有一个「分类图例」小区块放在「可视化」之前，作为阅读图示的字典。

**Epic 结构是作者主导维护的区块**。AI 按下述策略协助：

- **作者已手改入新开发项** → AI 按作者版本为准，**不再覆盖或调整**该区块；进入第一步直接渲染。
- **本轮有新开发项但 Epic 结构里没有** → AI 提议追加（说明往哪个叶 Epic 加 + 理由，或建议新建叶 Epic），询问作者并等其确认 / 调整；得到确认后由 AI 写回该区块、再渲染。
- **本轮 Epic 结构与现状一致** → 跳过提议步骤，直接渲染。

「可视化」和「节点索引」两个区块仍由 AI 整块替换（每次从最新 Epic 结构完全重建）。
...
**表格一律用紧凑单空格**：生成「分类图例」「节点索引」等表格时，每个单元格只用**一个空格**包裹、竖线不对齐（`| a | b |`、分隔行 `| - | - |`），**绝不**把各列补空格对齐到等宽。原因：DEVTREE.md 已被 `.prettierignore` 豁免（prettier 不会替它对齐），而对齐补空格会让「内容一变整列重排」→ 每次 `/devtree` 刷新 diff 爆炸；紧凑单空格下新增/改一行只动那一行。渲染结果两种写法完全相同（VSCode preview 里一模一样），差别只在底层字节与 diff 噪音。

> 注意：下方示例表在本 SKILL.md 里显示为**对齐**形态，是因为 SKILL.md 自身不在豁免名单、被 prettier 补齐了——示例仅供看**列结构**，实际写进 DEVTREE.md 时务必按上条用紧凑单空格。
