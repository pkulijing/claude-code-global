...
**没有更轻档，也不再往下调思考档**：真正琐碎的已在 Step 2 跳过；没跳过的（配置、指令规则文件）每行都重。**角度数是检出率的主驱动，不能砍**；而 `medium` 已是 Sonnet 编码场景的底 —— 官方分模型建议里 `low` 明确只适用于 chat 等非编码场景，再降一档省下的钱要拿编码判断力去付。且本 loop 的成本大头是**误报引发的无效修复轮**，压误报靠置信闸与探针验证，不靠减 reviewer。
