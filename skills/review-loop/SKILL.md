...
委派失败（Agent 工具不可用 —— 如 Codex 端 / 受限环境；或子 agent 起不来 / 返回的不是 finding 列表）→ 主会话按 Step 4 的角度清单逐角度过一遍 diff + 同一置信 rubric 过滤，告知用户「本次未走独立 context，主 context 会因此增大」，并在结果**顶部显著标注**：
