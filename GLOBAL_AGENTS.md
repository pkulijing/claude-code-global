...
- **`rules/lark.md`** — 触发：用 lark-cli 创作 / 编辑飞书云文档，或为 lark-cli 申请 / 排查授权。（署名约定 + docx 实操技巧 + 授权与 scope 管理）
- **`rules/shell.md`** — 触发：生成 / 编辑 bash / shell 脚本，或给用户需手动粘贴执行的 shell 命令。（中文 / 全角字符 × 引号与变量名两个固定坑 + 只在远端 / CI 跑的脚本必须配本地沙盘测试 + 给人的长命令一律写成脚本）
...
- 执行：按照计划，完成开发。Agent 主导，人类适当干预辅助。**执行前必须先完成 PROMPT.md 和 PLAN.md 的撰写并确认，再开始写代码。** 执行中 Agent 自主判断开发单元完成即 `/commit` 收口，**每次 commit 前自动走 review 循环**（`/review-loop`：委派独立 context 的子 agent 编队 review、硬 diff 升重档、委派不可用则降级本端结构化自审并标注、琐碎可跳过、2 轮不收敛留痕放行）迭代到「运行验证通过 + 无高置信 correctness 问题」——见「核心开发模式」。
  ...
  - **wrapper 原则：包装外部 CLI / 工具前，先核实它的原生能力**。动手写封装之前先读一遍或实测被包装工具的**参数解析与能力边界**；wrapper 只补它**真正缺的编排**（定位、起环境、收敛错误、报告），其余参数**原样透传**，别替它重造语义。否则容易基于臆测重复造轮子——真实代价：一层自实现的「逐包多步编排 + 失败隔离」建立在「被包装脚本一次只认一个包」这个未经核实的前提上，核实后发现多包 / 连带依赖 / 失败隔离原生全会做，整层厚封装被删、推倒重做为薄封装。
